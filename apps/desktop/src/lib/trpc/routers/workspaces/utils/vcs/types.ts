...
export interface VcsProvider {
  ...
  getBranchWorkspacePath(params: { mainRepoPath: string; branch: string }): Promise<string | null>;

  // --- Branch/bookmark operations ---
  getCurrentBranch(repoPath: string): Promise<string | null>;
  listBranches(repoPath: string, options?: { fetch?: boolean }): Promise<{ local: string[]; remote: string[] }>;
  ...
  branchExistsOnRemote(repoPath: string, branch: string): Promise<BranchExistsOnRemoteResult>;
  ...
  // --- Status & changes ---
  getAheadBehindCount(params: { repoPath: string; defaultBranch: string }): Promise<{ ahead: number; behind: number }>;
